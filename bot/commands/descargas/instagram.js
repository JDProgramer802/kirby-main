...
export default {
  name: 'instagram',
  aliases: ['ig', 'insta'],
  description: 'Descarga reel/post de Instagram (API cobalt)',
  category: 'Descargas',
  usage: '/instagram <url>',
  cooldown: 20,
  adminOnly: false,
  ...
  async execute(sock, msg, args, _db, _config) {
    ...
    if (!url || !/instagram\.com/i.test(url)) {
      return sock.sendMessage(remote, { text: formatError('Pega un enlace de Instagram.') });
    }
    try {
      ...
      await sock.sendMessage(remote, {
        text: formatError(e instanceof Error ? e.message : 'Falló Instagram'),
      });
    }
  },
};
