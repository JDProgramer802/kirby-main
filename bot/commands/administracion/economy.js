...
export default {
  name: 'economy',
  aliases: ['economia'],
  description: 'Activa o desactiva economía (alias de /toggleconomy)',
  category: 'Administración',
  usage: '/economy on|off|status',
  cooldown: 3,
  adminOnly: true,
  ownerOnly: false,
  groupOnly: true,
  ...
  async execute(sock, msg, args, db, _config) {
    ...
    if (sub !== 'on' && sub !== 'off') {
      await sock.sendMessage(remote, { text: formatError('Uso: /economy on | off | status') });
      return;
    }
    const on = sub === 'on';
    await db.query(
    ...
    await sock.sendMessage(remote, {
      text: formatSuccess(on ? '¡Economía encendida! 💰' : 'Economía apagada.'),
    });
  },
};
